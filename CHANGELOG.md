...
- **协议帧自适应分片**：基准测试（10 万文件、哈希开启）暴露出一个真实缺陷——
  ...
  `SendEntries` / `SendRelations` 按真实编码大小分片：超限即对半缩小重试，
  `ErrFrameTooLarge` 在写出任何字节前检测、重试不留半帧；单条数据连一帧
  都放不下时报错而不是丢数据（丢一条会让快照看似完整实则缺数据）。
- **扫描子进程 stderr 诊断**：Broker 保留子进程 stderr 的有界滚动尾
  （2 KiB），连接失败时以脱敏形式附在错误信息里（路径、邮箱、长十六进制
  串替换为占位符），崩溃不再只表现为 EOF；原始 stderr 字节不进入日志。
