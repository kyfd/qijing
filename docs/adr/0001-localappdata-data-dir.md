...
- 旧数据超过预算或空间不足时跳过迁移并记录标记，绝不能让应用无法启动；
  完整性检查以只读方式打开，不改写旧目录。
...
| 失败 | 行为 |
| --- | --- |
| 旧库 quick_check 失败 | 尽力归档后放弃迁移、全新开始，标记记 `legacy_db_unrecoverable`；旧目录原样 |
| 旧数据超过迁移预算 | 跳过迁移、全新开始，标记记 `legacy_skipped_too_large`；旧目录原样 |
| 磁盘剩余空间不足（需约 2 倍旧数据 + 余量） | 跳过迁移、全新开始，标记记 `legacy_skipped_insufficient_space`；旧目录原样 |
| 复制中断/瞬时 I/O 错误 | staging 清理，报错；旧目录原样；下次启动重试 |
| 备份校验失败 | 放弃迁移，报错 |
| 切换前 data 已存在 | 保留新位置数据（不覆盖），补写标记 |
| 标记写入失败 | 下次启动重新执行迁移；因复制是幂等的，结果一致 |

原则：**无法有效迁移的旧数据绝不能让应用无法启动**。跳过迁移只影响
扫描历史（可重新扫描得到），不影响应用功能；旧目录始终保留为备份。
只有瞬时 I/O 错误保持硬失败，以便腾出空间后重试。
