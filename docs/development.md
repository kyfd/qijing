...
2. 应用永不删除、移动、重命名或写入授权目录中的文件与目录。
3. 默认扫描只访问元数据。
4. 只有用户对授权根开启本地哈希时，scanner 才能读取文件字节；原始字节仅用于本地摘要计算。
5. 默认不联网；任何外部 Agent 请求都必须显式选择加入，并使用字段 allowlist 生成匿名 payload。
6. Agent 输出不能决定授权、路径边界或文件操作。
7. 测试不得扫描开发者真实的桌面、下载、文档、源码集合或整个磁盘；应使用临时夹具。

不要为了实现“建议”而添加删除、移动、重命名、覆盖、解压、Git 写操作或 shell 命令执行器。Explorer 定位仅负责显示目标。
...
只读保证不能仅凭代码中没有明显的 `Remove` 或 `Rename` 来判断；应以能力审计和不变性测试共同证明。
